    ...
    <div className="container max-w-4xl mx-auto px-6">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {steps.map((step, i) => (
          ...
          >
            <span className="gold-gradient-text font-display text-4xl font-bold opacity-40 absolute top-4 right-6">
              {step.num}
            </span>
            <h3 className="font-display text-xl text-gold mb-3">{step.title}</h3>
            <p className="text-section-dark-foreground/70 font-body font-light text-sm leading-relaxed">
              {step.text}
            </p>
          </motion.div>
        ))}
